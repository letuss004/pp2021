...
class Function4(Frame):
    ...
    def command_bt_dob_up(self):
        controller.sort_st_list_by_dob_up()
        self.destroy_inf()
        self.show_inf()
        self.button_dob.config(text="Student DoB  ↓",
                               command=lambda: [controller.sort_st_list_by_dob_down(), self.command_bt_dob_down()])
        pass

    def command_bt_dob_down(self):
        controller.sort_st_list_by_dob_down()
        self.destroy_inf()
        self.show_inf()
        self.button_dob.config(text="Student DoB  ↑",
                               command=lambda: [controller.sort_st_list_by_dob_up(), self.command_bt_dob_up()])
        pass

    def command_bt_name_up(self):
        controller.sort_st_list_by_name_up()
        self.destroy_inf()
        self.show_inf()
        self.button_name.config(text="Student Name  ↓",
                                command=lambda: [controller.sort_st_list_by_name_down(), self.command_bt_name_down()])
        pass

    def command_bt_name_down(self):
        controller.sort_st_list_by_name_down()
        self.destroy_inf()
        self.show_inf()
        self.button_name.config(text="Student Name  ↑",
                                command=lambda: [controller.sort_st_list_by_name_up(), self.command_bt_name_up()])
        pass

    def command_bt_id_up(self):
        controller.sort_st_list_by_id_up()
        self.destroy_inf()
        self.show_inf()
        self.button_id.config(text="Student ID  ↓",
                              command=lambda: [controller.sort_st_list_by_id_down(), self.command_bt_id_down()])

    def command_bt_id_down(self):
        controller.sort_st_list_by_id_down()
        self.destroy_inf()
        self.show_inf()
        self.button_id.config(text="Student ID  ↑",
                              command=lambda: [controller.sort_st_list_by_id_up(), self.command_bt_id_up()])
